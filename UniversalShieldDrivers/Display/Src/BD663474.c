    ...
    if (c > 127) c -= 96; else c -= 32;
		...
		for(uint8_t byteNumber = 0; byteNumber < currentFont -> width; byteNumber++) {
				...
				for(uint8_t bitNumber = 0; bitNumber < 8; bitNumber++) {
						...
						} else if(textBackColor != TFT_COLOR_none) { //Печать фона если он включен
							TFT_fillRectangle(TFT_cursorX+byteNumber*currentFontSize, TFT_cursorY+bitNumber*currentFontSize, currentFontSize, currentFontSize, textBackColor); 
						}
				}
		}
...
void TFT_print(uint16_t x, uint16_t y, char str[]) {
    TFT_cursorX = x;
    TFT_cursorY = y;
    uint16_t i = 0;
    while(str[i] != '\0') {
